...
{
    public enum HealthHeroRejectReason : byte
    {
        None = 0,
        UnknownStep = 1,
        AlreadyDone = 2
    ...
    /// <summary>
    /// Host-authoritative Health Hero shared state (U10 — pure replication of
    ...
    /// mid-attempt joins the in-progress attempt. Submissions arriving host-side
    /// after completion are ignored silently (completion guard).
    ///
    /// P13: the host seeds <see cref="ShuffleSeed"/> (tool-tray order) at spawn
    ...
    ///
    /// Step ordering (clipboard → tool → care plan) is deterministic LOCAL
    /// content, enforced client-side like Design Build's wrong-slot rule; the
    /// host validates step identity, duplicates, and completion only.
    /// </summary>
    public class HealthHeroNetworkState : NetworkBehaviour
    {
        ...
        /// </summary>
        public event Action<int, int, HealthHeroRejectReason> StepRejected;
        ...
        public HealthHeroRejectReason LastRejectReason { get; private set; } = HealthHeroRejectReason.None;
        ...
        public override void OnNetworkSpawn()
        {
            ...
            {
                _shuffleSeed.Value = ContentShuffle.NextSeed(0, HealthHeroClinicLayout.PieceIds.Length);
            }
        }
        ...
        public PlacementSubmissionResult ApplySubmission(int stepIndex, int submissionId, ulong senderClientId)
        {
            ...
            {
                SendReject(senderClientId, stepIndex, submissionId, HealthHeroRejectReason.UnknownStep);
                return PlacementSubmissionResult.Rejected;
            }

            if (_completedSteps.Contains(stepIndex))
            {
                SendReject(senderClientId, stepIndex, submissionId, HealthHeroRejectReason.AlreadyDone);
                return PlacementSubmissionResult.Rejected;
            }
        ...
        private void SendReject(ulong senderClientId, int stepIndex, int submissionId, HealthHeroRejectReason reason)
        {
        ...
        public void ResetForAttempt()
        {
            ...
            _shuffleSeed.Value = ContentShuffle.NextSeed(_shuffleSeed.Value, HealthHeroClinicLayout.PieceIds.Length);
        }
        ...
        /// <summary>Care-step index for a piece id (-1 for non-step pieces like the bandage).</summary>
        public static int StepIndexFor(string pieceId)
        {
            return IndexIn(HealthHeroClinicLayout.StepPieceIds, pieceId);
        }

        public static string StepPieceIdFor(int stepIndex)
        {
            var steps = HealthHeroClinicLayout.StepPieceIds;
        ...
        {
            return IndexIn(HealthHeroClinicLayout.PieceIds, pieceId);
        }

        public static string PieceIdFor(int pieceIndex)
        {
            var pieces = HealthHeroClinicLayout.PieceIds;
