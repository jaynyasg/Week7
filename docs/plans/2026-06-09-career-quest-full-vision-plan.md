...
- R5. A fallback asset path must keep the game playable if a generated sprite is missing.
...
flowchart TB
  ...
  Avatar --> Connection["Connection / Solo Fallback"]
  Connection --> Hub["Playable Campus Hub"]
...
- There is no behavior regression for entry, avatar, connection, Showcase, gallery, reveal, or quit.
...
- Add validation tests for required IDs, fallback presence, texture dimensions, and category coverage.
...
- Keep UI destination buttons only as accessibility/backup controls if they do not dominate the screen.
...
- Screenshot evidence shows avatar, guide, buildings, destination labels, and reveal/gallery access.

### U5. Shared Activity Framework

**Goal.** Create one lifecycle and state contract that every activity can use for longer solo and multiplayer loops.
...
- Provide a thin adapter so existing controllers can migrate one by one.

**Acceptance.**

- Shared tests prove duplicate completion records once.
- Activity controllers can be migrated without changing the `MiniGameResult` constructor contract.
...
- Render a real room/workbench and visible city pieces using `AssetCatalog`.
...
- Activity controllers will grow more stateful; the shared activity framework is needed to keep multiplayer and duplicate-result behavior consistent.
